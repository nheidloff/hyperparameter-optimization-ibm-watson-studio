...
tb_directory = os.environ["RESULT_DIR"]+"/"+os.environ["SUBID"]+"/logs/tb"
tf.gfile.MakeDirs(tb_directory)
print('Tensorboard dir: ' + tb_directory)
# HPO - end
...
def export_model(model_info, class_count, saved_model_dir):
  ...
  with graph.as_default():
    ...
    saved_model_dir = saved_model_dir + '/'+ str(instance_id) # HPO workaround since os.environ["SUBID"] not available
...
def main(_):
  ...
  with tf.Session(graph=graph) as sess:
    ...
    train_writer = tf.summary.FileWriter(tb_directory + '/train',
                                         sess.graph)
    validation_writer = tf.summary.FileWriter(tb_directory+'/test')
    # HPO - end
    ...
    for i in range(how_many_training_steps): # HPO
      ...
      is_last_step = (i + 1 == how_many_training_steps) # HPO
